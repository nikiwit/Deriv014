...
export const MyOnboardingProcess: React.FC = () => {
  const { user } = useAuth();
  ...
  const fetchDocStatus = async () => {
    ...
    try {
      ...
      console.log('[MyOnboardingProcess] Doc status response:', res.status);

      if (res.ok) {
        const data = await res.json();
        console.log('[MyOnboardingProcess] Doc status data:', data);
        setDocsState({
  ...
  const fetchContractPreview = async () => {
    ...
    try {
      ...
      const data = await res.json();
      console.log('[MyOnboardingProcess] Contract preview:', data);
      if (data.status === 'ok') {
  ...
  const downloadContractPdf = async () => {
    ...
    try {
      ...
      console.log('[MyOnboardingProcess] Contract saved to temp_data, generating PDF...');
...
export default MyOnboardingProcess;
