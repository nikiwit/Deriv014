...
export const MyDocuments: React.FC = () => {
  ...
  const fetchDocStatus = async () => {
    ...
    try {
      setStatusLoading(true);
      const res = await fetch(`${API_BASE}/api/employee-docs-status/${encodeURIComponent(employeeId)}`);
      console.log('[MyDocuments] Doc status response:', res.status);

      if (res.ok) {
        const data = await res.json();
        console.log('[MyDocuments] Doc status data:', data);
        setDocsState({
  ...
  const downloadContractPdf = async () => {
    // If contract already exists (signed via chat), download PDF directly
    if (docsState.contract.exists) {
      window.open(`${API_BASE}/api/download-contract-pdf/${encodeURIComponent(employeeId)}`, '_blank');
      return;
    }
    ...
    try {
      ...
      // Download PDF (backend will generate from saved JSON/contracts table)
      window.open(`${API_BASE}/api/download-contract-pdf/${encodeURIComponent(employeeId)}`, '_blank');
