...
export const Onboarding: React.FC = () => {
  ...
  const [preliminaryData, setPreliminaryData] = useState<Record<
    string,
    any
  > | null>(null);
    ...
    const fetchEmployees = async () => {
      try {
        ...
        const mapped: OnboardingJourney[] = data.employees.map((emp) => {
          ...
          if (
            emp.progress &&
            typeof emp.progress === "string" &&
            emp.progress.includes("/")
          ) {
  ...
  const stats = {
    ...
    avgProgress:
      employees.length > 0
        ? Math.round(
            employees.reduce((sum, e) => sum + e.progress, 0) /
              employees.length,
          )
        : 0,
  };
      ...
      setTimeout(() => {
        setMessages((prev) => [
          ...prev,
          {
            id: Date.now().toString(),
            sender: "ai",
            text: "Perfect. I'm analyzing compliance requirements, visa needs (if applicable), and building the culture plan...",
            step: 8,
          },
        ]);
      }, 600);
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm flex flex-col justify-between">
            <div>
              ...
              <div className="flex items-baseline space-x-1">
                <span className="text-3xl font-bold text-derivhr-600">
                  {stats.avgProgress}%
                </span>
                <Badge
                  variant={
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-lg overflow-hidden">
          ...
          <div className="divide-y divide-slate-100">
            {filteredEmployees.map((emp) => (
              <div
                key={emp.id}
                className="p-4 hover:bg-slate-50 transition-colors flex items-center"
              >
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button
                    onClick={() =>
                      alert(`Reminder sent to ${emp.employeeName}!`)
                    }
  ...
  // OFFER GENERATOR MODE
  if (viewMode === "offer" && preliminaryData) {
    return (
  ...
  if (aiMode) {
    ...
    const handleAIFinish = (data: OnboardingData, analysisResult: string) => {
      ...
      // Navigate to Offer Generator instead of list
      setViewMode("offer");
    };
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] gap-6">
      {/* Chat Column */}
      <div
        className={`flex flex-col h-full transition-all duration-500 ease-in-out ${analysis ? "w-1/3" : "w-full max-w-2xl mx-auto"}`}
      >
        ...
        {!analysis && (
          <div className="mb-8 text-center space-y-2 animate-fade-in">
            ...
            <h1 className="text-3xl font-black text-slate-900 tracking-tighter">
              AI Onboarding Architect
            </h1>
            <p className="text-slate-500 font-medium">
              I'll guide you through creating a personalized, compliant
              onboarding journey.
            </p>
          </div>
        )}
        ...
        {/* Chat Area */}
        <div
          className="flex-1 overflow-y-auto space-y-6 pr-4 pb-4 scroll-smooth"
          ref={scrollRef}
        >
          {messages.map((msg) => (
            <div
            ...
            >
              <div
                className={`max-w-[85%] p-4 rounded-2xl text-sm leading-relaxed shadow-sm animate-fade-in ${
                  msg.sender === "user"
              ...
              >
                {msg.sender === "ai" && (
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <span className="text-[10px] font-black text-derivhr-500 uppercase tracking-widest">
                      DerivHR AI
                    </span>
                  </div>
                )}
        ...
        {/* Input Area */}
        <div className="pt-4 pb-2">{renderControls()}</div>
      </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-xs font-bold text-slate-500 flex items-center uppercase tracking-wide">
                  ...
                  <span className="text-derivhr-500 ml-1">
              ...
              {analysis.split("\n").map((line, i) => {
                if (line.startsWith("##"))
                  return (
                    <h2
                      key={i}
                      className="text-2xl font-black text-slate-800 mt-6 mb-4 pb-2 border-b border-slate-100 tracking-tight"
                    >
                      {line.replace("##", "").trim()}
                    </h2>
                  );
                if (line.startsWith("###"))
                  return (
                    <h3
                      key={i}
                      className="text-lg font-bold text-derivhr-600 mt-4 mb-2 tracking-tight uppercase text-xs"
                    >
                      {line.replace("###", "").trim()}
                    </h3>
                  );
                ...
                if (line.trim().startsWith("-"))
                  return (
                    <div key={i} className="flex items-start mb-2 ml-1">
                      ...
                      <span className="text-slate-600 leading-relaxed text-sm font-medium">
                        {line.replace("-", "").trim()}
                      </span>
                    </div>
                  );
                return (
                  <p
                    key={i}
                    className="mb-2 text-slate-600 text-sm font-medium leading-relaxed"
                  >
                    {line}
                  </p>
                );
          ...
          <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-slate-400">
            <div className="flex items-center space-x-4">
              <span className="flex items-center">
                <CheckCircle2 size={14} className="mr-1 text-jade-500" />{" "}
                Compliance Checked
              </span>
              <span className="flex items-center">
                <Globe size={14} className="mr-1 text-purple-500" /> Strategy
                Applied
              </span>
            </div>
