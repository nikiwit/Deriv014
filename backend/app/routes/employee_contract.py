...
def process_contract_request(employee_context: Optional[dict] = None, session_id: Optional[str] = None) -> dict:
    ...
    if not user_id:
        return {
            "status": "error",
            "response": "3Unable to identify your user profile. Please log in again.",
        }
    ...
    if not db_user:
        return {
            "status": "error",
            "response": f"user id: {user_id}, email: {email}" + "Unable to identify your user profile. Please log in again.",
        }
