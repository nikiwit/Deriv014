...
def download_contract_pdf(employee_id):
    ...
    # PDF doesn't exist - regenerate from signed contract data
    try:
        db = get_db()
        data = None

        # 1. Try to get data from contracts table (signed contract)
        try:
            contract_result = (
                db.table("contracts")
                .select("*")
            ...
            if contract_result.data:
                ...
                # contract_data column has the nested structure from sign_and_store_contract
                raw = contract_row.get("contract_data")
                if raw:
                    nested = json.loads(raw) if isinstance(raw, str) else raw
                    employee_sec = nested.get("employee", {})
                    job_sec = nested.get("job", {})
                    company_sec = nested.get("company", {})

                    # Build flat pdf_data matching what _generate_contract_pdf expects
                    data = {
                        "fullName": employee_sec.get("fullName") or contract_row.get("employee_name", ""),
                        "nric": employee_sec.get("nric") or contract_row.get("nric", ""),
                        "passportNo": "",
                        "nationality": employee_sec.get("nationality") or contract_row.get("nationality", ""),
                        "dateOfBirth": employee_sec.get("dateOfBirth", ""),
                        "gender": "",
                        "maritalStatus": "",
                        "race": "",
                        "religion": "",
                        ...
                        "city": "",
                        "state": "",
                        "country": "Malaysia" if contract_row.get("jurisdiction") == "MY" else "Singapore",
                        "personalEmail": "",
                        "workEmail": employee_sec.get("email") or contract_row.get("email", ""),
                        "mobile": "",
                        "altNumber": "",
                        "emergencyName": "",
                        ...
                        "emergencyMobile": "",
                        "emergencyAltNumber": "",
                        "jobTitle": job_sec.get("job_name") or contract_row.get("position", ""),
                        "department": job_sec.get("department") or contract_row.get("department", ""),
                        "reportingTo": job_sec.get("reporting_to", ""),
                        "startDate": str(employee_sec.get("startDate") or contract_row.get("start_date", "")),
                        "employmentType": job_sec.get("employment_type", "Full-Time Permanent"),
                        "probationPeriod": f"{contract_row.get('probation_months', 3)} months",
                        "workModel": "",
                        "workLocation": "",
                        "monthlySalary": str(employee_sec.get("offered_salary") or contract_row.get("salary", "")),
                        "currency": company_sec.get("currency") or contract_row.get("currency", "MYR"),
                        "bankName": nested.get("banking", {}).get("bankName", ""),
                        "accountHolder": nested.get("banking", {}).get("accountHolder") or contract_row.get("employee_name", ""),
                        "accountNumber": nested.get("banking", {}).get("accountNumber", ""),
                        "bankBranch": "",
                        "taxResident": True,
                        ...
                        "signatureDate": (contract_row.get("employee_signed_at") or "")[:10],
                        "completedAt": contract_row.get("employee_signed_at", ""),
                        "id": employee_id,
                    }
        except Exception as e:
            logger.warning(f"Could not load contract from DB for {employee_id}: {e}")

        # 2. Fallback: try JSON file from sign_and_store_contract
        if not data:
            ...
            if json_path.exists():
                ...
                company_sec = nested.get("company", {})
                data = {
                    "fullName": employee_sec.get("fullName", ""),
                    "nric": employee_sec.get("nric", ""),
                    "passportNo": "",
                    "nationality": employee_sec.get("nationality", ""),
                    "dateOfBirth": employee_sec.get("dateOfBirth", ""),
                    "gender": "",
                    "maritalStatus": "",
                    "race": "",
                    ...
                    "city": "",
                    "state": "",
                    "country": "Malaysia" if company_sec.get("jurisdiction") == "MY" else "Singapore",
                    "personalEmail": "",
                    "workEmail": employee_sec.get("email", ""),
                    "mobile": "",
                    "altNumber": "",
                    "emergencyName": "",
                    "emergencyRelationship": "",
                    "emergencyMobile": "",
                    "emergencyAltNumber": "",
                    "jobTitle": job_sec.get("job_name", ""),
                    "department": job_sec.get("department", ""),
                    "reportingTo": job_sec.get("reporting_to", ""),
                    "startDate": str(employee_sec.get("startDate", "")),
                    "employmentType": job_sec.get("employment_type", "Full-Time Permanent"),
                    "probationPeriod": f"{job_sec.get('probation_period_months', 3)} months",
                    "workModel": "",
                    "workLocation": "",
                    "monthlySalary": str(employee_sec.get("offered_salary", "")),
                    "currency": company_sec.get("currency", "MYR"),
                    "bankName": nested.get("banking", {}).get("bankName", ""),
                    "accountHolder": nested.get("banking", {}).get("accountHolder", ""),
                    "accountNumber": nested.get("banking", {}).get("accountNumber", ""),
                    "bankBranch": "",
                    "taxResident": True,
                    "acknowledgeHandbook": True,
                    ...
                    "acknowledgeConfidentiality": True,
                    "finalDeclaration": True,
                    "signatureDate": nested.get("consent_confirmed_at", "")[:10] if nested.get("consent_confirmed_at") else "",
                    "completedAt": nested.get("completedAt", ""),
                    "id": employee_id,
                }
...
def get_contract_preview(employee_id):
    """
    Get contract preview data. If contract is already signed, returns the signed
    contract data from the contracts table/JSON. Otherwise builds preview from user data.
    """
    try:
        ...
        # Check if contract is already signed (use stored contract_data)
        ...
        try:
            ...
            if contract_result.data:
                signed_contract = contract_result.data[0]
        except Exception:
            pass

        # Also check for signed contract JSON file
        json_path = TEMP_DIR / f"{employee_id}_contract.json"
        signed_json = None
        if json_path.exists():
            try:
        ...
        # If we have a signed contract, return its data
        if signed_contract or signed_json:
            # Prefer DB contract_data, fall back to JSON file
            if signed_contract and signed_contract.get("contract_data"):
                raw = signed_contract["contract_data"]
                contract_data = json.loads(raw) if isinstance(raw, str) else raw
            elif signed_json:
                contract_data = signed_json
            else:
                contract_data = {}

            # Ensure nested structure exists (contract_data from sign_and_store_contract is nested)
            ...
            # Fill in banking from contract or DB
            banking = contract_data.get("banking", {})
            if not banking.get("bankName") and signed_contract:
                banking = {
                    "bankName": signed_contract.get("bank_name", ""),
                    "accountHolder": signed_contract.get("employee_name", ""),
                    "accountNumber": signed_contract.get("bank_account_number", ""),
                }

            # Build terms from signed contract or defaults
            jurisdiction = company_sec.get("jurisdiction") or (signed_contract or {}).get("jurisdiction", "MY")
            from app.document_generator import _get_jurisdiction_defaults
            defaults = _get_jurisdiction_defaults(jurisdiction)

            preview = {
                "employee": employee_sec,
                "job": job_sec,
                "company": company_sec,
                "banking": banking,
                "terms": contract_data.get("terms", {
                    "workHours": (signed_contract or {}).get("work_hours") or defaults["work_hours"],
                    "noticePeriod": (signed_contract or {}).get("notice_period") or defaults["notice_period"],
                    "governingLaw": (signed_contract or {}).get("governing_law") or defaults["governing_law"],
                ...
                "schema": schema,
                "status": "signed",
                "signedAt": (signed_contract or {}).get("employee_signed_at") or contract_data.get("consent_confirmed_at", ""),
            }

            return jsonify({
                "status": "ok",
                "contractData": preview,
                "canSign": False,  # Already signed
                "isSigned": True,
            })

        # No signed contract - build preview from user data
        ...
        contract_data = {
            "employee": {
                ...
                "startDate": str(user.get("start_date", "")),
                "offered_salary": str(user.get("salary", "")),
            ...
            "job": {
                ...
                "job_name": user.get("position_title") or user.get("role", ""),
                "department": user.get("department", ""),
                "employment_type": "Full-Time Permanent",
                "role_summary": user.get("role_summary", ""),
                ...
                "benefits": {
                    ...
                    "medical_insurance": {
                        "annual_limit": 50000 if jurisdiction == "MY" else 80000,
                    },
                ...
                "probation_period_months": int(defaults.get("probation_months", 3)),
                "career_path": [],
            },
            "company": {
